      ...
      for (final doc in snapshot.docs) {
        try {
          ...
          if (!data.containsKey('image') || data['image'] == null) {
            _logger.warning(
              'Skipping service ${doc.id}: Missing required field "image"',
            );
            continue;
          }
          if (!data.containsKey('features') || data['features'] == null) {
            _logger.warning(
              'Skipping service ${doc.id}: Missing required field "features"',
            );
            continue;
          }
          ...
          if (data['features'] is! List) {
            _logger.warning(
              'Skipping service ${doc.id}: "features" must be a list',
            );
            continue;
          }
